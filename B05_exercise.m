...
% Rayleigh waves propagating inside a thin 
% medium, located between the free surface and a high velocity layer. Those
% waves degenerate to Lamb waves, which can propagate a number of modes,
% either symmetrical or antisymmetrical.
...
model.recx  = [50:100:(model.x(end)-50)];
Nreceivers  = numel(model.recx);

model.recz  = ones(1,Nreceivers) * model.z(end)-170;
model.dtrec = 0.004;
