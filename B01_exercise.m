...
model.vel=zeros(Nz,Nx)+2000;      % initialize matrix

% ----------------------------------------
% 2. Source parameters (in the center of the model)

source.x    = 200;   %[model.x(end)/2 ];
source.z    = 200;   %[model.z(end)/2 ]; 
source.f0   = [80 ];
source.t0   = [0.04  ];
source.amp  = [100 ];
...
model.recz  = 40:20:400;  % ... a trick to have same nr elements  of recx
